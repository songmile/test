...
<div class="container-fluid h-100 ha_2 ha_1">
    <h5 class="text-center ha_2 ha_1">系统用户列表</h5>
    <div id="toolbar">
        <div class="input-group col-xs-12">
            <input class="form-control ha_2 ha_1" placeholder="请输用户名" id="qryName"/>
                ...
                <div class="modal-body">
                    <form id="editForm" role="form" class="form-horizontal">
                        ...
                        <div class="form-group form-item-custom">
                            ...
                            <div class="col-sm-10 btn-group" id="divStatus" data-toggle="buttons">
                                <label class="col-2 btn btn-default">
                                    <input class="toggle" type="radio" name="status" value="0">正常
                                </label>
                                <label class="col-2 btn btn-default">
                                    <input class="toggle" type="radio" name="status" value="1">禁用
                                </label>
                            </div>
                        </div>
                    </form>
...
<script>
    ...
    $(function (){
        ...
        $("#btnSava").click(function(){
            ...
            let postData={
                ...
                "status": $('#divStatus input:radio:checked').val(),
            };
            //执行异步提交
            $.ajax({
                type: "post",
    ...
    function fillData(rowData) {
        ...
        $("#divStatus").find("input:radio[value='" + rowData.status + "']").prop("checked", "checked");
        $("#divPassword").show();
        $("#btnSava").show();
        $("#myModal").modal();
    }
    ...
    function ajaxRequest(params) {
        $.ajax({
            url: '${pageContext.request.contextPath}/user/paged',
            type: "post",
            dataType: "json",
            contentType: "application/json;charset=UTF-8",
                    ...
                    events: {
                        'click #rowDelete': function (e, value, row, index) {//行删除按钮的单击事件
                            layer.confirm('您确定要删除吗？', {
                                ...
                                $.ajax({
                                    type: "DELETE",
                                    url: "${pageContext.request.contextPath}/user/delete/" + row.id,
                                    dataType: "json",
                                    cache: false,
                                    async: false,
